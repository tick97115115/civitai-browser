...
describe('Test ModelId View', () => {
  test('test modelId endpoint validation model', async () => {
    const result = modelId_response(modelId_json)
    if (result instanceof type.errors) {
      result.throw()
    } else {
      expect(result.id).eq(11821)
    }
  })

  test('test validation', () => {
    const wrapper = mount(ModelIdView, { props: { modelIdResponse: modelId_json } })
    expect(wrapper.text()).toContain(modelId_json.id)
  })
})
